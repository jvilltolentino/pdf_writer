...
def setup_logger(output_dir: str) -> tuple[logging.Logger, str]:
    ...
    The log file is named 'execution_YYYY-MM-DD_HH-MM-SS.log' so each run
    gets its own log without overwriting older ones.
    """
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    log_path = os.path.join(output_dir, f"execution_{timestamp}.log")

    # Use a unique logger name per run so multiple runs don't share handlers
    logger = logging.getLogger(f"pdf_filler_{timestamp}")
    logger.setLevel(logging.INFO)
    logger.propagate = False  # don't bubble up to root logger

    # Clear any old handlers (safety for repeated runs)
...
class ExcelPdfFillerApp(tk.Tk):
    ...
    def _run_generation(self, active_map: dict, name_field: str):
        ...
        # Set up the per-run log file in the output folder
        logger, log_path = setup_logger(self.output_dir)

        # Log a header so the file is easy to read later
        logger.info("=" * 60)
        logger.info("PDF GENERATION RUN STARTED")
        logger.info("=" * 60)
        ...
        # Close the file handler so the log file is flushed and released
